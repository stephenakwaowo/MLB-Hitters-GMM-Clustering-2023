              ...
              fluidRow(
                box(title = "Select Players", width = 12, 
                    selectInput("player1_dropdown", "Player 1", choices = sort(unique(merged_success$Name)),
                                selected = sample(unique(merged_success$Name), 1)),
              ...
              fluidRow(
                box(title = "Methodology", width = 12, 
                    ...
                    tags$div(style = "margin-left: 20px;",
                             h4(HTML("<b>Sequential Dimensionality Reduction Strategy</b>")),
                             ...
                             p("GMM can model clusters with different covariance structures. Although GMM supports various covariance types, in this project, the 'full' covariance type was selected. This type allows each cluster to have its own general covariance matrix, enabling the model to capture correlations between features within each cluster, which is critical for accurately representing the data structure."),
                    ...
                    tags$div(style = "margin-left: 20px;",
                             ...
                             h4(HTML("<b>Interpretation</b>")),
                             p("The BONDS Index does not have a strict cutoff for 'good' or 'bad' scores, as the context of the league and era can shift these perceptions. However, within the context of the 2023 Season:"),
                             tags$ul(
                               tags$li(HTML("<b>Lower Scores (below 44.12):</b> Players with scores below 44.12 generally struggle in multiple key offensive areas of BONDS, such as making strong contact, showing good power, or avoiding poor pitch selections. These scores place them in the bottom quartile of the distribution.")),
                               tags$li(HTML("<b>Average Scores (44.12 to 49.12):</b> Scores between 44.12 and 49.12 indicate average performance across the measured metrics. Players within this range may show strengths in some areas but also have weaknesses in others.")),
                               tags$li(HTML("<b>Good Scores (49.12 to 55.36):</b> Scores between 49.12 and 55.36 indicate above-average performance, with players excelling in some key metrics while still maintaining strong overall performance.")),
                               tags$li(HTML("<b>Very Good Scores (55.36 to 63.88):</b> Scores between 55.36 and 63.88 indicate strong overall performance, approaching elite status.")),
                               tags$li(HTML("<b>Elite Scores (above 63.88):</b> Players with scores above 63.88 are in the 95th percentile and higher, representing the top performers in power, discipline, and effective batted-ball outcomes. These players are often among the league's best hitters, resembling the characteristics of elite hitters like Barry Bonds."))
                             ),
                             
                             h4(HTML("<b>Limitations</b>")),
                             p("While the BONDS Index is a robust metric, it has limitations:"),
                             tags$ul(
                               tags$li(HTML("<b>Swing Decisions:</b> The metric is not the most dynamic when it comes to the swing decision aspect of the metric. It does not account for the count of a plate appearance or the quality of pitches. For example, swinging at an in-zone 0-0 curveball is typically less valuable and less likely to result in a positive outcome compared to swinging at an in-zone four-seamer on a 3-0 count.")),
                               tags$li(HTML("<b>Era and Context:</b> The index does not adjust for differences in league or era, which can influence the relative value of certain skills (e.g., power hitting in a high-offense era)."))
                             ),
                             h4(HTML("<b>Conclusion</b>")),
                             p("The BONDS Index serves as a comprehensive metric for evaluating offensive performance, drawing inspiration from one of the greatest hitters in baseball history, Barry Bonds. It offers a nuanced view of a player's abilities, helping to identify key areas of strength and opportunities for improvement.")
...
server <- function(input, output, session) {
  ...
  weights <- c(
    ...
    'Spd' = 0.15 * scaling_factor,
    'BsR' = 0.4 * scaling_factor
  )
  ...
  output$bonds_index_gauge <- renderPlotly({
    ...
    if (nrow(player_data) == 0) {
      return(NULL)  # Handles case where player data is not found
    }
